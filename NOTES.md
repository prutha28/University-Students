...
Every HTTP request or response has a certain format. It consists of two parts - the headers and an optional payload(message body). 
...
|-----------------------------------|
|	Host: www.google.com 			|
|   Date: Sat, 09 Feb 2015			|	<------ Optional Headers ( All headers are optional except the "Host" header, which specifies which host you are trying to access)
...
The first line is called the request line and consists of the HTTP verb, the file path( That's the path where the file lies on the server) and the version of HTTP being used. This is then followed by a set of headers. Headers consists of all the meta information/additional information that the client can send to the server. A header is a key value pair separated by a :. 

A request can also contain a body in the request. This is not needed in case of GET & DELETE requests. However, in case of POST and PUT requests, the request body usually contains the message body.


## Response

|-----------------------------------|
|	HTTP/1.1 200 OK					|	<------ Status Line ( HTTP Version, Status Code, Reason phrase or the status info in plain english)
|-----------------------------------|
...
- When the browser parses, the HTML markup, it automatically creates what we call the "DOM Tree", or the Document Object Model. Each "tag" in the html mark up corresponds to a new "node" in the DOM Tree. 

- The rules for creating the DOM Tree from the HTML markup is defined as a set of rules in the HTML specification. The markup is broken down into a set of tokens by the "Tokenizer" as per the rules defined in the specificaion.
...
- We use AJAX to send asynchronous requests to the server. AJAX is  a set of web technologies that let you make requests programtically.
...
<p>An operation in general (or an HTTP method in particular is said to be idempotent), if performing the operation multiple times has the same effect on the system, when it was performed the very first time. 
...
While PUT requires the user to specify the entire resource( including the fields with changed values, as well as those with the same values) in the request body. PATCH only needs the request body to contain those fileds of the entity whose values are to be changed.

# URL( Uniform Resource Locator)

<p>A URL is a formatted string that determines where a particular file is located, on the internet. A typical URL looks like this. This is what you typically type in your browser. 
	<pre>
...
|		:9000				| Port that server is serving optional 						|
...
2. Speicifying the Port number is optional. If no port number is specified, it is port 80 by default.
3. This can sometimes be followed by an optional set of query parameters. Query Parameters or 'GET' parameters serve as additional filters for the response from the server. The server can make use of this additional information in order to process the request.Query Paramaters are in the form of key value pairs separted from each other by '&'. 
<pre>e.g. http://www.ncsu.edu:9000/csc505/handout.pdf?semester=spring&year=2015<pre>

When the user types in the URL in the browser, a request is first sent to the DNS( Domain Name Server) server that sends back the IP address of the server that corresponds to the domain name. This is then followed by the actual request to the server for the content.
</p>

# URI ( Uniform Resource Identifier)
URI is very similar to URLs. But URLs are generally used more in reference to web applications while URIs are used with reference to APIs.
